...
getDefraMapConfig().then((defraMapConfig) => {
  ...
  const floodMap = new FloodMap('map', {
    ...
    queryArea: {
      ...
      onShapeUpdate: ({ area, geometry }) => {
        ...
        mapState.boundaryAreaHa = Math.round(area / 10000 * 10) / 10
        return {
          warningText: !isValid ? warningText : null,
          allowShape: true
        }
  ...
  const mapState = {
    ...
    isFloodZone: false,
    boundaryAreaHa: null,
    viewExtentZones: null,
    viewExtent: null,
    centrePoint: null,
    centreFloodZone: null
  }

  const floodZoneQueryMinScale = 250000

  const debugLog = (msg) => {
    const el = document.getElementById('map-debug-log')
    if (el) el.textContent = msg
  }

  const roundEN = (n) => Math.round(n)

  const queryFloodZonesInExtent = async () => {
    const { scale, extent } = floodMap.view
    if (!extent) {
      mapState.viewExtent = null
      mapState.centrePoint = null
      mapState.centreFloodZone = null
      mapState.viewExtentZones = null
      updateMapDescription(mapState)
      return
    }
    const { xmin, ymin, xmax, ymax } = extent
    mapState.viewExtent = {
      xmin: roundEN(xmin), ymin: roundEN(ymin),
      xmax: roundEN(xmax), ymax: roundEN(ymax)
    }
    mapState.centrePoint = {
      easting: roundEN((xmin + xmax) / 2),
      northing: roundEN((ymin + ymax) / 2)
    }

    if (!mapState.isFloodZone || scale > floodZoneQueryMinScale) {
      mapState.viewExtentZones = null
      mapState.centreFloodZone = null
      updateMapDescription(mapState)
      return
    ...
    const { width, height } = floodMap.view

    // Sample a dense grid of screen points to detect zones across the extent.
    // Zone 2 renders beneath Zone 3, so we need enough points to hit Zone 2-only
    // pixels at the outer edges where Zone 3 is absent.
    const samplePoints = []
    const steps = [0.03, 0.12, 0.22, 0.33, 0.44, 0.5, 0.56, 0.67, 0.78, 0.88, 0.97]
    for (const fx of steps) {
      for (const fy of steps) {
        samplePoints.push({ x: Math.round(width * fx), y: Math.round(height * fy) })
      }
    }
    const centreIndex = Math.floor(samplePoints.length / 2) // centre of grid

    try {
      const hitResults = await Promise.all(
        samplePoints.map(pt => floodMap.view.hitTest(pt, { include: layersToTest }))
      )

      const allZones = hitResults.flatMap(result =>
        result.results.map(r => {
          const attrs = r.graphic?.attributes
          if (!attrs) return null
          const feature = transformFeature({ isPixelFeaturesAtPixel: true, items: [attrs] })
          return feature ? getFloodZoneFromFeature(feature, mapState) : null
        }).filter(Boolean)
      )

      mapState.viewExtentZones = [...new Set(allZones)]

      // Centre point specifically
      const centreResult = hitResults[centreIndex] // index 12 = centre of 5x5 grid (0.5, 0.5)
      const centreFeatures = centreResult.results
      if (centreFeatures.length) {
        const attrs = centreFeatures[0].graphic?.attributes
        const feature = attrs && transformFeature({ isPixelFeaturesAtPixel: true, items: [attrs] })
        mapState.centreFloodZone = feature ? getFloodZoneFromFeature(feature, mapState) : 'none'
      } else {
        mapState.centreFloodZone = 'none'
      }

      debugLog(`sampleZones: ${JSON.stringify(mapState.viewExtentZones)}\ncentreZone: ${mapState.centreFloodZone}\ncentreHits: ${centreResult.results.length}`)
    } catch (e) {
      console.log('[fmp-debug] hitTest error', e)
      debugLog(`hitTest error: ${e.message}`)
      mapState.viewExtentZones = null
      mapState.centreFloodZone = null
    }
    updateMapDescription(mapState)
  ...
  const getDatasetLabel = (segments) => {
    if (segments.includes('fzcl')) return 'Flood zones 2 and 3 (2070 to 2125)'
    if (segments.includes('fzpd') || segments.includes('fz')) return 'Flood zones 2 and 3'
    if (segments.includes('sw')) return 'Surface water'
    if (segments.includes('mo')) return 'None'
    return 'Unknown'
  }

  const getTimeframeLabel = (segments) => {
    if (segments.includes('fzcl')) return null
    if (segments.includes('cl')) return 'Climate change (2061 to 2125)'
    return 'Present day'
  }

  const getLikelihoodLabel = (segments) => {
    if (!segments.includes('sw')) return null
    if (segments.includes('hr')) return '1 in 30 (high)'
    if (segments.includes('mr')) return '1 in 100 (medium)'
    if (segments.includes('lr')) return '1 in 1000 (low)'
    return null
  }

  const updateMapDescription = (state, queryParts = []) => {
    const el = document.getElementById('map-description')
    if (!el) return
    const parts = [`Dataset: ${getDatasetLabel(state.segments)}`]
    const timeframe = getTimeframeLabel(state.segments)
    if (timeframe) parts.push(`Timeframe: ${timeframe}`)
    const likelihood = getLikelihoodLabel(state.segments)
    if (likelihood) parts.push(`Annual likelihood of flooding: ${likelihood}`)
    if (state.viewExtent) {
      const { xmin, ymin, xmax, ymax } = state.viewExtent
      parts.push(`Map view extent: SW ${xmin}E ${ymin}N to NE ${xmax}E ${ymax}N`)
    }
    if (state.centrePoint) {
      const { easting, northing } = state.centrePoint
      let centrePart = `Map centre: ${easting}E ${northing}N`
      if (state.centreFloodZone === 'none') {
        centrePart += ' (not in a flood zone)'
      } else if (state.centreFloodZone === '2' || state.centreFloodZone === 'FZ2') {
        centrePart += ' (within Flood Zone 2)'
      } else if (state.centreFloodZone === '3' || state.centreFloodZone === 'FZ3') {
        centrePart += ' (within Flood Zone 3)'
      } else if (state.centreFloodZone === terms.keys.fzCC) {
        centrePart += ' (within flood zones plus climate change)'
      } else if (state.centreFloodZone === terms.keys.fzNoData) {
        centrePart += ' (climate change data unavailable)'
      } else if (state.centreFloodZone) {
        centrePart += ` (within ${state.centreFloodZone})`
      }
      parts.push(centrePart)
    }
    if (state.boundaryAreaHa !== null) {
      parts.push(`Site boundary confirmed: ${state.boundaryAreaHa} hectares. Use 'Get summary report' to see which ${getDatasetLabel(state.segments)} data intersects this boundary.`)
    }
    if (state.viewExtentZones !== null) {
      if (state.viewExtentZones.length === 0) {
        parts.push('Current map view: no flood zone areas visible')
      } else {
        const zoneLabels = state.viewExtentZones.map(z => {
          if (z === 'FZ2' || z === '2') return 'Flood Zone 2'
          if (z === 'FZ3' || z === '3') return 'Flood Zone 3'
          if (z === terms.keys.fzCC) return 'Flood zones plus climate change'
          if (z === terms.keys.fzNoData) return 'Climate change data unavailable'
          return z
        })
        parts.push(`Current map view contains: ${zoneLabels.join(' and ')}`)
      }
    }
    if (queryParts.length) {
      parts.push('Selected location —')
      queryParts.forEach(([key, value]) => parts.push(`${key}: ${value}`))
    }
  ...
  mapDiv.addEventListener('appaction', e => {
    const { type } = e.detail
    if (type === 'confirmPolygon') {
      updateMapDescription(mapState)
    }
    if (type === 'deletePolygon') {
      mapState.boundaryAreaHa = null
      updateMapDescription(mapState)
    }
  })
  ...
  floodMap.addEventListener('ready', async e => {
    ...
    floodMap.view.watch('stationary', (isStationary) => {
      if (isStationary) {
        queryFloodZonesInExtent()
      }
    })
...
  
  ...
  floodMap.addEventListener('query', e => {
    ...
    */
    ...
    updateMapDescription(mapState, listContents)
  })
})
